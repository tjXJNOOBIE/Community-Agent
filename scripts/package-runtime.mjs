...
const source = resolve(root, 'build', 'install', 'community-agent')
...
if (result.status !== 0) throw new Error(`Community Agent Java distribution failed with exit code ${result.status}`)
...
writeFileSync(resolve(root, 'runtime-manifest.json'), `${JSON.stringify({
  schemaVersion: 1,
  product: '@tjxjnoobie/community-agent',
  packageVersion: packageJson.version,
  distribution: 'community-agent',
  launchers: ['community-agent'],
  treeSha256: tree.digest('hex'),
  files: entries,
}, null, 2)}\n`)
console.log(`Packaged Community Agent Java runtime: ${entries.length} files, ${entries.reduce((sum, entry) => sum + entry.bytes, 0)} bytes`)
