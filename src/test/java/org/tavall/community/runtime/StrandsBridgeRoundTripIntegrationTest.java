...
class StrandsBridgeRoundTripIntegrationTest {
    ...
    @Test
    void standaloneBridgeConnectsBackToAuthorizedJavaFunctionCatalog() {
        ...
        assertThat(nodeExecutable)
                .as("%s must be configured when the bridge integration is required", NODE_ENV)
                .isNotBlank();
        assertThat(bridgeEntrypoint)
                .as("%s must be configured when the bridge integration is required", ENTRYPOINT_ENV)
                .isNotBlank();
        ...
        AIFunctionCatalogView authorizedView = new AIFunctionCatalogView(
                catalog,
                function -> "integration_visible".equals(function.getName())
        );
        ...
        try (AIFunctionMcpHttpServer functionServer = AIFunctionMcpHttpServer.start(authorizedView);
            ...
            agent.put("id", "community-java-round-trip");
            agent.put("name", "Community Java Round Trip");
            ...
            javaFunctions.put("toolFilters", Map.of(
                    "allowed", List.of("integration_visible", "integration_hidden")
            ));

            Map<String, Object> runtimeConfig = new LinkedHashMap<>();
            runtimeConfig.put("agent", Map.copyOf(agent));
            runtimeConfig.put("mcpServers", Map.of("java-authority", Map.copyOf(javaFunctions)));

            bridgeClient.createAgent(runtimeConfig);
            bridgeClient.closeAgent("community-java-round-trip");
        } finally {
    ...
    static final class IntegrationFunctions {
        @AIFunction(
                name = "integration_visible",
                description = "Visible only to the authorized Strands integration view."
        )
        ...
        @AIFunction(
                name = "integration_hidden",
                description = "Registered in the root catalog but excluded from the authorized integration view."
        )
