...
const nodeEnvironment = 'COMMUNITY_AGENT_STRANDS_NODE'
const entrypointEnvironment = 'COMMUNITY_AGENT_STRANDS_ENTRYPOINT'
...
const assertPackage = () => {
  if (!existsSync(runtime) || !existsSync(manifestPath)) throw new Error('Java runtime bundle is missing; install a published Community Agent package or build the release package with npm pack.')
  const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'))
  if (manifest.product !== '@tjxjnoobie/community-agent') throw new Error('Community Agent runtime manifest has the wrong product identity.')
  ...
  if (JSON.stringify(actual) !== JSON.stringify(manifest.files)) throw new Error('Community Agent runtime integrity verification failed.')
  const tree = createHash('sha256')
  for (const entry of actual) tree.update(`${entry.path}\0${entry.sha256}\n`)
  if (tree.digest('hex') !== manifest.treeSha256) throw new Error('Community Agent runtime tree verification failed.')
  return manifest
}
...
const assertJava = () => {
  ...
  if (result.error || result.status !== 0) throw new Error('Java 25 or newer is required to run the Java Community Agent backend.')
...
const doctor = () => {
  ...
  console.log('Community Agent doctor: PASS')
  ...
  console.log('MCP: Java operator/internal HTTP surfaces available through community-agent serve')
  console.log('Discord/provider credentials: not inspected by the package doctor')
}
...
const launch = (args) => {
  ...
  assertJava()
  const bridge = resolveBridgeEntrypoint()
  const invokedAs = basename(process.argv[1] ?? 'community-agent')
  const backend = resolve(runtime, 'bin', `community-agent${process.platform === 'win32' ? '.bat' : ''}`)
